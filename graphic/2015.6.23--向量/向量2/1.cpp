...
void GameRun()
{
	...
	Ellipse(back_dc, pos.x - _R, pos.y - _R, pos.x + _R, pos.y + _R);
	BitBlt(main_dc, 0, 0, _CLIENT_W, _CLIENT_H, back_dc, 0, 0, SRCCOPY);

	//把速度的大小慢慢从一个正数变为0，方向不变，可以这样模拟摩擦力
	...
	if (!_IS_ZERO(speed.Length()))
	{
		...
		CVector2 speed_dir = speed.Normalize();
		speed_len -= 0.05f;
	...
	if (GetAsyncKeyState(VK_LBUTTON) & 1)
	{
		...
		{
			//得到点中的位置的向量
			CVector2 click((float)p.x, (float)p.y);

			//得到球心到点中的位置的方向向量
			CVector2 dir = click - pos;

			//更新速度，速度的方向应该和dir
			//一样，但是大小由我们自己确定
			//speed = dir.Normalize() * _SPEED;
			speed = _SPEED * dir.Normalize();
		}
	}
}
