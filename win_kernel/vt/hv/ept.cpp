...
namespace ept
{
	...
	/// <returns></returns>
	static unsigned __int8 get_memory_type(unsigned __int64 pfn, bool is_large_page)
	{
		unsigned __int64 page_start_address = is_large_page ? pfn * LARGE_PAGE_SIZE : pfn * PAGE_SIZE;
		unsigned __int64 page_end_address = is_large_page ? (pfn * LARGE_PAGE_SIZE) + (LARGE_PAGE_SIZE - 1) : (pfn * PAGE_SIZE) + (PAGE_SIZE - 1);
	...
	/// <param name="pfn"></param>
	/// <returns></returns>
	static bool isValidForLargePage(unsigned __int64 pfn)
	{
	...
	static bool setupPml2Entry(__ept_pde& entry, unsigned __int64 pfn)
	{
		...
		{
			void* split_buffer = PoolManager::requestPool<void*>(PoolManager::INTENTION_SPLIT_PML2, true, sizeof(__ept_dynamic_split));
			if (split_buffer == nullptr)
			{
				LOG_ERROR("Failed to allocate split buffer");
				return false;
			}

			return splitPml2(split_buffer, pfn * LARGE_PAGE_SIZE);// pfn << 21
		}
	}
	...
	static bool createEptPageTable()
	{
		PHYSICAL_ADDRESS max_size;
		max_size.QuadPart = MAXULONG64;

		g_vmm_context->ept_state->ept_page_table = allocateContignousMemory<__vmm_ept_page_table>();
		if (g_vmm_context->ept_state->ept_page_table == NULL)
		{
			LOG_ERROR("Failed to allocate memory for PageTable");
			return false;
		}
		...
		for (int i = 0; i < 512; i++)
		{
			for (int j = 0; j < 512; j++)
			{
				if (!setupPml2Entry(page_table->pml2[i][j], (i * 512) + j))
				{
					return false;
				}
			}
	...
	static __ept_pde* getPml2Entry(unsigned __int64 physical_address)
	{
		...
		if (pml4_index > 0)
		{
			LOG_ERROR("Address above 512GB is invalid");
			return nullptr;
		}
	...
	/// <param name="physical_address"></param>
	/// <returns></returns>
	static __ept_pte* getPml1Entry(unsigned __int64 physical_address)
	{
		unsigned __int64 pml4_index = MASK_EPT_PML4_INDEX(physical_address);
		unsigned __int64 pml3_index = MASK_EPT_PML3_INDEX(physical_address);
		unsigned __int64 pml2_index = MASK_EPT_PML2_INDEX(physical_address);
	...
	static bool isPageSplitted(unsigned __int64 physical_address)
	{
		__ept_pde* entry = getPml2Entry(physical_address);
	...
	static void hookWriteAbsoluteJump(unsigned __int8* target_buffer, unsigned __int64 destination_address)
	{
	...
	static void hookWriteRelativeJump(unsigned __int8* target_buffer, unsigned __int64 destination_address, unsigned __int64 source_address)
	{
	...
	/// <returns></returns>
	static bool hookInstructionMemory(__ept_hooked_function_info* hooked_function_info, void* target_function, void* hooked_function,void* trampoline, void** origin_function)
	{
		...
		if (trampoline != 0)
		{
			...
			{
				LOG_ERROR("Function at page boundary");
				return false;
			}
			...
			hookWriteRelativeJump(&hooked_function_info->fake_page_contents[page_offset], (unsigned __int64)trampoline, (unsigned __int64)target_function);

			RtlCopyMemory(hooked_function_info->first_trampoline_address, target_function, hooked_instructions_size);

			hookWriteAbsoluteJump(&hooked_function_info->first_trampoline_address[hooked_instructions_size], (unsigned __int64)target_function + hooked_instructions_size);

			*origin_function = hooked_function_info->first_trampoline_address;

			return hookFunction(trampoline, hooked_function, nullptr, nullptr);
		}
		...
		{
			hookWriteAbsoluteJump(&hooked_function_info->fake_page_contents[page_offset], (unsigned __int64)hooked_function);
			return true;
		}
	...
	void buildMtrrMap()
	{
		...
		if (mtrr_def_type.mtrr_enabled == 0)
		{
		...
		if (mtrr_cap.fixed_range_support == 1 && mtrr_def_type.fixed_range_mtrr_enabled)
		{
	...
	bool initialize()
	{
		__eptp* ept_pointer = allocatePool<__eptp*>(PAGE_SIZE);
		if (ept_pointer == NULL)
		{
			return false;
		}
	...
	bool splitPml2(void* pre_allocated_buffer, unsigned __int64 physical_address)
	{
		__ept_pde* entry = getPml2Entry(physical_address);
		if (entry == NULL)
		{
			LOG_ERROR("Invalid address passed");
			return false;
		}
	...
	void swapPml1(__ept_pte* entry_address, __ept_pte entry_value)
	{
		// Acquire the lock
	...
	void swapPml1AndInvalidateTlb(__ept_pte* entry_address, __ept_pte entry_value, invept_type invalidation_type)
	{
		// acquire the lock
		spinlock::lock(&g_vmm_context->ept_state->pml_lock);
	...
	bool hookFunction(void* target_function, void* hooked_function,void* trampoline, void** origin_function)
	{
		unsigned __int64 physical_address = MmGetPhysicalAddress(target_function).QuadPart;
		if (physical_address == NULL)
		{
			LOG_ERROR("Requested virtual memory doesn't exist in physical one");
			return false;
		}
		...
		while (&g_vmm_context->ept_state->hooked_page_list != current->Flink)
		{
			...
			{
				LOG_INFO("Page already hooked");

				__ept_hooked_function_info* hooked_function_info = PoolManager::requestPool<__ept_hooked_function_info*>(PoolManager::INTENTION_TRACK_HOOKED_FUNCTIONS, TRUE, sizeof(__ept_hooked_function_info));
				if (hooked_function_info == nullptr)
				{
					LOG_ERROR("There is no pre-allocated pool for hooked function struct");
					return false;
				}
				...
				if(origin_function != nullptr)
				{
					hooked_function_info->first_trampoline_address = PoolManager::requestPool<unsigned __int8*>(PoolManager::INTENTION_EXEC_TRAMPOLINE, TRUE, 100);
					if (hooked_function_info->first_trampoline_address == nullptr)
					{
						PoolManager::releasePool(hooked_function_info);
						LOG_ERROR("There is no pre-allocated pool for trampoline");
						return false;
					}
				}
				...
				if (hookInstructionMemory(hooked_function_info, target_function, hooked_function, trampoline, origin_function) == false)
				{
					if (hooked_function_info->first_trampoline_address != nullptr)
					{
						PoolManager::releasePool(hooked_function_info->first_trampoline_address);
					}
		...
		if (!isPageSplitted(physical_address))
		{
			void* split_buffer = PoolManager::requestPool<void*>(PoolManager::INTENTION_SPLIT_PML2, true, sizeof(__ept_dynamic_split));
			if (split_buffer == nullptr)
			{
				LOG_ERROR("There is no preallocated pool for split");
				return false;
			}

			if (!splitPml2(split_buffer, physical_address))
			{
				PoolManager::releasePool(split_buffer);
				LOG_ERROR("Split failed");
				return false;
			}
		}

		__ept_pte* target_page = getPml1Entry(physical_address);
		if (target_page == nullptr)
		{
			LOG_ERROR("Failed to get PML1 entry of the target address");
			return false;
		}

		__ept_hooked_page_info* hooked_page_info = PoolManager::requestPool<__ept_hooked_page_info*>(PoolManager::INTENTION_TRACK_HOOKED_PAGES, true, sizeof(__ept_hooked_page_info));
		if (hooked_page_info == nullptr)
		{
			LOG_ERROR("There is no preallocated pool for hooked page info");
			return false;
		}
		...
		if (hooked_function_info == nullptr)
		{
			PoolManager::releasePool(hooked_page_info);
			LOG_ERROR("There is no preallocated pool for hooked function info");
			return false;
		}
		...
		hooked_function_info->second_trampoline_address = trampoline;
		hooked_function_info->fake_page_contents = hooked_page_info->fake_page_contents;
		if(!hookInstructionMemory(hooked_function_info, target_function, hooked_function, trampoline, origin_function))
		{
			if (hooked_function_info->first_trampoline_address != nullptr)
			{
				PoolManager::releasePool(hooked_function_info->first_trampoline_address);
	...
	bool unhookFunction(unsigned __int64 virtual_address)
	{
			...
			if (hooked_page_info->pfn_of_hooked_page == GET_PFN(physical_address))
			{
				...
				while (&hooked_page_info->hooked_functions_list != current_hooked_function->Flink)
				{
					...
					if (function_page_offset == MASK_EPT_PML1_OFFSET(hooked_function_info->virtual_address))
					{
						...
						{
							unhookFunction((unsigned __int64)hooked_function_info->second_trampoline_address);
						}
						...
						if (hooked_page_info->hooked_functions_list.Flink == hooked_page_info->hooked_functions_list.Blink)
						{
							...
							swapPml1AndInvalidateTlb(hooked_page_info->entry_address, hooked_page_info->original_entry, INVEPT_SINGLE_CONTEXT);
	...
	void unhookAllFunctions()
	{
		...
		while (&g_vmm_context->ept_state->hooked_page_list != current_hooked_page)
		{
			...
			swapPml1AndInvalidateTlb(hooked_entry->entry_address, hooked_entry->original_entry, INVEPT_SINGLE_CONTEXT);

			RemoveEntryList(current_hooked_page);

			current_hooked_page = current_hooked_page->Flink;

			PoolManager::releasePool(hooked_entry);
		}
	}
}