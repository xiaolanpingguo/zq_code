...
namespace hvgt 
{
	static void broadcastVmoff(KDPC* Dpc, PVOID DeferredContext, PVOID SystemArgument1, PVOID SystemArgument2)
	{
		LOG_INFO("call vm off cpu num:%d\n", KeGetCurrentProcessorNumber());
		UNREFERENCED_PARAMETER(Dpc);
		UNREFERENCED_PARAMETER(DeferredContext);

		__vm_call(VMCALL_VMXOFF, 0, 0, 0);

		LOG_INFO("vm will be offed, cpu num:%d\n", KeGetCurrentProcessorNumber());
		KeSignalCallDpcSynchronize(SystemArgument2);
		KeSignalCallDpcDone(SystemArgument1);
	}

	static void broadcastInveptAllContexts(KDPC* Dpc, PVOID DeferredContext, PVOID SystemArgument1, PVOID SystemArgument2)
	{
		UNREFERENCED_PARAMETER(Dpc);
		UNREFERENCED_PARAMETER(DeferredContext);
	...
	static void broadcastInveptSingleContext(KDPC* Dpc, PVOID DeferredContext, PVOID SystemArgument1, PVOID SystemArgument2)
	{
		UNREFERENCED_PARAMETER(Dpc);
		UNREFERENCED_PARAMETER(DeferredContext); 
	...
	void vmoff()
	{
		KeGenericCallDpc(broadcastVmoff, NULL);
	}
	...
	{
		if (invept_all == true) KeGenericCallDpc(broadcastInveptAllContexts, NULL);
		else KeGenericCallDpc(broadcastInveptSingleContext, NULL);
	}
	...
	bool perform_memory_allocation()
	{
		...
		{
			LOG_ERROR("Couldn't get hypervisor device object pointer");
			return false;
		}
