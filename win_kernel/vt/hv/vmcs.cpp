...
void setPrimaryControls(__vmx_primary_processor_based_control& primary_controls) 
{
	/**
...
void setSecondaryControls(__vmx_secondary_processor_based_control& secondary_controls) 
{
	/**
...
static void setEntryControl(__vmx_entry_control& entry_control) 
{
	/**
...
static void setExitControl(__vmx_exit_control& exit_control) 
{
	/**
...
static void setPinbasedControlMsr(__vmx_pinbased_control_msr& pinbased_controls) 
{
	/**
...
static void setExceptionBitmap(__exception_bitmap& exception_bitmap)
{
...
static unsigned __int64 getSegmentBase(unsigned __int16 selector, unsigned __int8* gdt_base)
{
...
static void fillGuestSelectorData(void* gdt_base, unsigned __int32 segment_register, unsigned __int16 selector)
{
	if (selector & 0x4)
	{
		return;
	...
	{
		// (segment_limit + 1) * PAGE_SIZE - 1
		// => segment_limit * PAGE_SIZE + PAGE_SIZE - 1 
		// => segment_limit * 0x1000 + 0xfff
...
static unsigned __int32 ajdustControls(unsigned __int32 ctl, unsigned __int32 msr)
{
	__msr msr_value = { 0 };
...
void fillVmcs(__vcpu* vcpu, void* guest_rsp)
{
	...
	setEntryControl(entry_controls);
	setExitControl(exit_controls);
	setPrimaryControls(primary_controls);
	setSecondaryControls(secondary_controls);
	setExceptionBitmap(exception_bitmap);
	setPinbasedControlMsr(pinbased_controls);
	...
	// you can set a msr hijack here
	//hv::setMsrBitmap(0xC0000082, vcpu, true, true, true);
	...
	hv::setIoBitmap(0x5655, vcpu, false);
	hv::setIoBitmap(0x5656, vcpu, false);
	hv::setIoBitmap(0x5657, vcpu, false);
	hv::setIoBitmap(0x5658, vcpu, false);
	hv::setIoBitmap(0x5659, vcpu, false);
	hv::setIoBitmap(0x565a, vcpu, false);
	hv::setIoBitmap(0x565b, vcpu, false);
	hv::setIoBitmap(0x1094, vcpu, false);
	hv::setIoBitmap(0x1090, vcpu, false);
	...
	// hypervisor features
	hv::vmwrite<unsigned __int64>(CONTROL_PIN_BASED_VM_EXECUTION_CONTROLS, ajdustControls(pinbased_controls.low, vmx_basic.true_controls ? IA32_VMX_TRUE_PINBASED_CTLS : IA32_VMX_PINBASED_CTLS));
	hv::vmwrite<unsigned __int64>(CONTROL_PRIMARY_PROCESSOR_BASED_VM_EXECUTION_CONTROLS, ajdustControls(primary_controls.low, vmx_basic.true_controls ? IA32_VMX_TRUE_PROCBASED_CTLS : IA32_VMX_PROCBASED_CTLS));
	hv::vmwrite<unsigned __int64>(CONTROL_SECONDARY_PROCESSOR_BASED_VM_EXECUTION_CONTROLS, ajdustControls(secondary_controls.low, IA32_VMX_PROCBASED_CTLS2));
	hv::vmwrite<unsigned __int64>(CONTROL_VM_EXIT_CONTROLS, ajdustControls(exit_controls.low, vmx_basic.true_controls ? IA32_VMX_TRUE_EXIT_CTLS : IA32_VMX_EXIT_CTLS));
	hv::vmwrite<unsigned __int64>(CONTROL_VM_ENTRY_CONTROLS, ajdustControls(entry_controls.low, vmx_basic.true_controls ? IA32_VMX_TRUE_ENTRY_CTLS : IA32_VMX_ENTRY_CTLS));

	// segments
	fillGuestSelectorData((void*)gdtr.base_address, ES, __read_es());
	fillGuestSelectorData((void*)gdtr.base_address, CS, __read_cs());
	fillGuestSelectorData((void*)gdtr.base_address, SS, __read_ss());
	fillGuestSelectorData((void*)gdtr.base_address, DS, __read_ds());
	fillGuestSelectorData((void*)gdtr.base_address, FS, __read_fs());
	fillGuestSelectorData((void*)gdtr.base_address, GS, __read_gs());
	fillGuestSelectorData((void*)gdtr.base_address, LDTR, __read_ldtr());
	fillGuestSelectorData((void*)gdtr.base_address, TR, __read_tr());
	...
	hv::vmwrite<unsigned __int64>(HOST_TR_BASE, getSegmentBase(__read_tr(),(unsigned char*)gdtr.base_address));
	...
	hv::vmwrite<void*>(GUEST_RIP, vmxRestoreState);
	hv::vmwrite<unsigned __int64>(HOST_RSP, (unsigned __int64)vcpu->vmm_stack + VMM_STACK_SIZE - PAGE_SIZE);
	hv::vmwrite<void*>(HOST_RIP, vmmEntrypoint);
	...
	if (secondary_controls.enable_vpid == 1)
	{
		hv::vmwrite<unsigned __int64>(CONTROL_VIRTUAL_PROCESSOR_IDENTIFIER, 1);
	}

	if (secondary_controls.enable_ept == 1 && secondary_controls.enable_vpid == 1)
	{
