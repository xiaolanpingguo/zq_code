...
void vmexit_vmcall_handler(__vcpu* vcpu) 
{
	...
	switch (vmcall_reason)
	{
		...
		case VMCALL_EPT_HOOK_FUNCTION:
		{
			...
			status = ept::hookFunction((void*)vmcall_parameter1, (void*)vmcall_parameter2, (void*)vmcall_parameter3, (void**)vmcall_parameter4);
		...
		case VMCALL_EPT_UNHOOK_FUNCTION:
		{
			...
			if (vmcall_parameter1 == 1)
			{
				ept::unhookAllFunctions();
			}

			else
			{
				// Page physciall address
				status = ept::unhookFunction(vmcall_parameter2);
			}
