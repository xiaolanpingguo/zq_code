...
//CCMenuItemImage：菜单项图像类
//create（static）：创建一个菜单项图像对象

//CCMenu：菜单类，用于装载菜单项图像类对象
//create（static）：创建一个菜单，注意本函数是不定
//                  参函数，并且内部会将填入的菜单
//                  项图像设置为创建的菜单的子节点

//CCLabelTTF：标签类，就是显示一个字符串
//create（static）：创建一个标签类对象

//CCSprite：精灵类，就是显示一个图像
//create（static）：创建一个精灵类对象

//CCSize：尺寸类，成员变量就是宽、高

//CCPoint：坐标类，成员变量就是x、y
//ccp(x,y)宏是返回一个CCPoint(x,y)
//CCPointZero就是CCPoint(0,0)
