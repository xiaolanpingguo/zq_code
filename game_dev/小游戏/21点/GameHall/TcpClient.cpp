...
void CTcpClient::_Send()
{
	...
	if (m_State != _CS_NO_CONNECTED && m_State != _CS_TRY_CONNECT && m_State != _CS_WAIT_FINISH)
	{
		...
		while (!m_SDQueue.Empty())
		{
			...
			{
				int r = send(g_Socket, sd.data + sd.send_size, sd.all_size - sd.send_size, 0);
				if (r == SOCKET_ERROR)
				{
					//真的出错
...
void CTcpClient::_Recv()
{
	...
	{
		//接收数据
		int r = recv(g_Socket, buf, 65536, 0);
						   ...
						   int intervalka)
{
	...
	ioctlsocket(g_Socket, FIONBIO, &no_blocking);
	...
	connect(g_Socket, (sockaddr*)&si, sizeof(si));
	...
	case _CS_TRY_CONNECT:
		{
			...
			if (1 == fs.fd_count)
			{
				...
				//设置心跳包及其时间
				BOOL b = TRUE;
				setsockopt(g_Socket, SOL_SOCKET, SO_KEEPALIVE, (const char*)&b, sizeof(b));
				DWORD dw;
				tcp_keepalive tka = {1, m_BeginKA, m_IntervalKA};
				WSAIoctl(g_Socket, SIO_KEEPALIVE_VALS, &tka, sizeof(tka), 0, 0, &dw, 0, 0);
