...
void GameRun()
{
	...
	for (std::list<CVector2>::iterator i = obj.begin(); i != obj.end(); )
	{
		Ellipse(back_dc, i->x - _R, i->y - _R, i->x + _R, i->y + _R);
		CVector2 p1(i->x, i->y);
		CVector2 dir = hezo - p1;
		speed = dir.Normalize() * 2.0f;
		(*i) += speed;
		RECT t;
		RECT t1 = { i->x - _R, i->y - _R, i->x + _R, i->y + _R };
		RECT t2 = { hezo.x - _R, hezo.y - _R, hezo.x + _R, hezo.y + _R };
		if (IntersectRect(&t, &t1, &t2))
			i = obj.erase(i);
		else
			++i;
	}
	Rectangle(back_dc, hezo.x - _R, hezo.y - _R, hezo.x + _R, hezo.y + _R);
	BitBlt(main_dc, 0, 0, _CLIENT_W, _CLIENT_H, back_dc, 0, 0, SRCCOPY);


	//按下鼠标左键
	if ((GetAsyncKeyState('A') & 0x8000) && hezo.x >= 0)
		hezo.x -= 5;
	if ((GetAsyncKeyState('D') & 0x8000) && hezo.x < _CLIENT_W)
		hezo.x += 5;
	if ((GetAsyncKeyState('W') & 0x8000) && hezo.y >= 0)
		hezo.y -= 5;
	if ((GetAsyncKeyState('S') & 0x8000) && hezo.y < _CLIENT_H)
		hezo.y += 5;
	if (GetAsyncKeyState(VK_LBUTTON) & 1)
	{
		...
		{
			CVector2 temp((float)(p.x), (float)(p.y));
			obj.push_back(temp);
		}
	}
}
