...
m_cDark(0)
{
	...
	m_iClientW = r.right - r.left;
	m_iClientH = r.bottom - r.top;
	...
	DeleteObject(hbmp);
	BitBlt(m_DarkDC, 0, 0, m_iClientW, m_iClientH, 0, 0, 0, BLACKNESS);
}
...
{
	BitBlt(m_BackDC, 0, 0, m_iClientW, m_iClientH, 0, 0, 0, WHITENESS);
}
...
void CGameOutput::End()
{
	if (m_cDark != 0)
	{
		BLENDFUNCTION bf = {AC_SRC_OVER, 0, m_cDark, 0};
		AlphaBlend(m_BackDC, 0, 0, m_iClientW, m_iClientH, m_DarkDC, 0, 0, m_iClientW, m_iClientH, bf);
	}
	BitBlt(m_MainDC, 0, 0, m_iClientW, m_iClientH, m_BackDC, 0, 0, SRCCOPY);
}
