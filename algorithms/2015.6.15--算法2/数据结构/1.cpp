...
//其它
//{
//	内存池（池表示的意思是释放资源的时候并不真正进行释放，而是保留为后面的服务做准备）
//}
...
void main()
{
	...
	for (int i = 0; i < 100; ++i)
	{
		void* p = mp.Allocate(rand() % 256 + 256);
		v.push_back(p);
	}
	...
	for (int i = 0; i < 100; ++i)
	{
		...
		int j = rand() % v.size();
		mp.Free(v[j]);
		v.erase(v.begin() + j); //只有vector的迭代器可以和整数进行加减
	}

	system("pause");

	//CMidSearchTree<int> st;
	//st.Insert(2);
	//st.Insert(8);
	//st.Insert(6);
	//st.Insert(4);
