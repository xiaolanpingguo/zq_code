...
awaitable<void> connecter()
{
	auto executor = co_await this_coro::executor;
	default_token::as_default_on_t<tcp::socket> socket(executor);
	asio::ip::tcp::endpoint endpoint(asio::ip::address::from_string("127.0.0.1"), 55556);
...
int main()
{
	try
	{
		...
		co_spawn(io_context, connecter(), detached);
