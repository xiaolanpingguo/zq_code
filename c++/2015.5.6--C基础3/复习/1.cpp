...
void main()
{
	...
	*((int*)(a4 + 3)) = a3;
	...
	//12）库函数
	//a：字符串 strlen、strcpy_s、strcat_s、strcmp、strchr、strstr
	//b：字符串类型转换 sprintf_s、itoa、atoi、ftoa、atof
	//c：时间 time、ctime_s
	//d：内存操作 memcpy、memset
	//e：数学 sin、asin、cos、acos、sqrt、abs
	//f：其它 srand、rand、system、exit、assert
	//g：堆 malloc、calloc、realloc、free
	//h：文件 fopen_s、fclose、fseek、ftell、rewind、fread、fwrite

	//13）断言
	//assert(表达式)：如果表达式为真则什么都不发生，表达式为假系统
	//                会强制用exit退出程序并给出断言失败的代码行号
	//                注意assert只是在debug版本中起作用
	//int a;
	//std::cin>>a;
	//assert(a > 5);
	...
	//13）结构体
	//内存对齐
	//支持的运算符：赋值、sizeof、条件运算符（放在?之后）、位、下标、取地址、解引用、对象引出、地址引出
	struct _ABC_
	{
		int a, b, c;
	};
	...
	_XYZ1* p1 = (_XYZ1*)t;
	_XYZ2* p2 = (_XYZ2*)t;
	//虽然p1和p2都指向同一段内存，但是对于这段内存的解读是不一样
